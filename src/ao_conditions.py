# Observing-condition presets keep observing difficulty separate from the
# numerical IntegrationConfig scale presets.

"""Observing-condition presets for public-data-informed AO demos."""
...
from data_sources import ALLOWED_SOURCE_CLASSES, EsoAsmSnapshot
...
class ObservingConditionConfig:
    """Observing/error condition for Notebook 11 public-data-informed runs.

    The numerical scale remains in ``IntegrationConfig``. This object controls
    atmosphere strength, photon budget, detector noise, latency, stroke, NCPA,
    and registration stress terms.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if int(self.latency_frames) < 0:
            raise AOConditionError("latency_frames must be >= 0.")
        if len(self.misregistration_shift_px) != 2:
            raise AOConditionError("misregistration_shift_px must contain two values.")
    ...
    def latency_total_s(self) -> float:
        return float(self.exposure_latency_s + self.readout_latency_s + self.compute_latency_s + self.dm_settling_latency_s)
    ...
    def integer_misregistration_shift_px(self) -> tuple[int, int]:
        return (int(round(self.misregistration_shift_px[0])), int(round(self.misregistration_shift_px[1])))
...
def phase_amplitude_from_seeing(
    ...
    _require_positive("reference_phase_amplitude_nm", reference_phase_amplitude_nm)
    return float(reference_phase_amplitude_nm * seeing_arcsec / reference_seeing_arcsec)


def r0_from_seeing_arcsec(seeing_arcsec: float, wavelength_m: float = 500.0e-9) -> float:
    """Fried-parameter conversion used for synthetic condition presets."""
...
def default_observing_conditions(
    ...
    """Build the Notebook 11 public-data-informed condition matrix."""

    _require_nonnegative("catalog_photons_per_subap_frame", catalog_photons_per_subap_frame)
    ...
    theta0_rad = theta0_rad_from_arcsec(float(measurements["theta0_arcsec"]))
    speed = float(measurements["turbulence_speed_ms"])
    atmosphere_source = "ESO ASM nighttime direct_public_data cache"
    common_note = (
        "Notebook 11 public-data-informed synthetic AO condition: atmosphere and/or "
        "catalog photometry may come from public caches, while DM, detector, latency, "
    ...
    return (
        ObservingConditionConfig(
            ...
            source_note="Notebook 11 nominal synthetic condition for regression comparison.",
        ),
        ObservingConditionConfig(
            ...
            photon_source="engineering 200 photons/subap/frame level, not measured telemetry",
            read_noise_e=1.0,
            latency_frames=1,
        ...
        ObservingConditionConfig(
            ...
            atmosphere_source="ESO ASM nighttime scaled engineering poor-seeing proxy",
        ...
        ObservingConditionConfig(
            ...
            theta0_rad=theta0_rad,
            turbulence_speed_m_s=speed,
            photons_per_subap_frame=max(1.0e-3, catalog_photons_per_subap_frame),
        ...
        ObservingConditionConfig(
            ...
            atmosphere_source="ESO ASM nighttime scaled engineering stress proxy",
...
def condition_rows(conditions: Sequence[ObservingConditionConfig]) -> list[dict[str, object]]:
    """Convert condition presets to CSV-ready dictionaries."""
