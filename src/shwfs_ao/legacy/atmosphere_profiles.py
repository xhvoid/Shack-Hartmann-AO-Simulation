...
from .data_sources import EsoAsmSnapshot, LiteratureAtmosphereProfile
...
class AtmosphereConfig:
    ...
    Args:
        ...
        tau0_s: Atmospheric coherence time in seconds. This is retained as an
            observation-summary/provenance field; the explicit layer winds and
            caller-supplied ``dt_s`` determine frozen-flow translations, so
            applying tau0 a second time would double-count temporal evolution.
    ...
    def __post_init__(self) -> None:
        ...
        _atmosphere_provenance(self.source_class, self.source_note)
        _assert_normalized_layer_weights(self.layers)

    @property
    def provenance(self) -> _Provenance:
        """Return the canonical provenance record for this atmosphere."""

        return _atmosphere_provenance(self.source_class, self.source_note)
...
def generate_multilayer_phase_cube(
    ...
    Physics note:
        ...
        speed, time, and grid sampling. ``config.tau0_s`` is reported metadata
        here; it is not an additional evolution multiplier because the layer
        winds already define the frozen-flow rate.
    """
...
def _atmosphere_provenance(source_class: str, source_note: str) -> _Provenance:
    ...
    try:
        ...
        raise AtmosphereProfileError("source_note must be a non-empty string.") from exc
