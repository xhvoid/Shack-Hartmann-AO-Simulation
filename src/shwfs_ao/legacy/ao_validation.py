...
class ValidationCheckResult:
    ...
    def provenance(self) -> _Provenance:
        """Return the canonical provenance record for this validation row."""

        return _validation_provenance(self.source_class, self.source_note)
...
class ValidationScanResult:
    ...
    @property
    def provenance(self) -> _Provenance:
        """Return the canonical provenance record for this validation scan."""
...
def _validate_source(source_class: str, source_note: str) -> None:
    _validation_provenance(source_class, source_note)


def _validation_provenance(source_class: str, source_note: str) -> _Provenance:
    try:
        ...
        raise AOValidationError("source_note must be a non-empty string.") from exc
